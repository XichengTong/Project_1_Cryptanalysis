...
def mono_decrypt(ciphertext, plaintexts):  
...
#The main decrypt function when it is poly_decrpt
def poly_decrypt(ciphertext, plaintexts):
    decrpted_collect=[]
    for plaintext in plaintexts:
        percentage_dict = load_local_percentage(plaintext)
...
def main():
    ...
    if cipher_type == "mono-alphabetic substitution cipher":
        result = mono_decrypt(ciphertext, all_plaintexts)
        ...
        result = poly_decrypt(ciphertext, all_plaintexts)
